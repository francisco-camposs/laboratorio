#include "funcionario.h"
	...
	// Método para alterar o salario do funcionário.
	// Recebe como parâmetro um valor em porcentagem.
	void funcionario::alterar_salario(float alteracao){
		salario = ((salario) * (alteracao/100+1));
	}

	// Compara se o CPF ou nome dos funcionários são iguais para identificar funcionários iguais.
	bool funcionario::operator==(funcionario func){
		if (CPF == func.get_CPF() || nome == func.get_Nome())
			return true;
		else
			return false;
	};